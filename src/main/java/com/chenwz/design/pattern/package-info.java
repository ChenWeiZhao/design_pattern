...
一、创建型模式
    ...
    1、工厂模式与抽象工厂模式 （Factory Pattern）（Abstract Factory Pattern）：
        不同条件下创建不同实例
    2、单例模式 （Singleton Pattern）：保证一个类仅有一个实例建造者模式 （Builder Pattern）：
...
二、结构型模式
    ...
    5、过滤器模式 （Filter、Criteria Pattern）：
...
三、行为型模式
    ...
    11、空对象模式（Null Object Pattern）：
        创建一个未对该类做任何实现的空对象类，该空对象类将无缝地使用在需要检查空值的地方。不要为了屏蔽null而使用空对象，应保持用null，远比用非null的值来替代“无值”要好。（慎用）

 */
package com.chenwz.design.pattern;
