/**
 * 文件名称: AbstractStrategyRouter.java
 * 文件描述:
 * 版权所有: 版权所有(C)2016-
 * 公    司: 金证财富南京科技有限公司
 * 内容摘要:
 * 其他说明:
 * 创建日期： 2021/1/28 11:26
 * 完成日期：
 * 修改记录1:
 * <pre>
 *    修改日期：
 ...
 * 如果是策略A1、A2、B1、B2 这样的叶子节点，由于不需要再向下委托，
 * 因此不再需要同时继承 AbstractStrategyRouter<T, R> 抽象类，只需要在 R apply(T param); 中实现业务逻辑即可。
 *
 * <pre>
 *           +---------+
 *           |  Root   |   ----------- 第 1 层策略入口
